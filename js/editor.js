...
export default class Editor {
    ...
    setupUI() {
        ...
        this.overlay.innerHTML = `
            <div id="workbench-header" style="position:absolute; top:0; left:0; width:100%; background:rgba(0,0,0,0.8); padding:10px; box-sizing:border-box; pointer-events:auto; border-bottom:1px solid #333;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    <div style="color:#0ff; font-size:1.2rem;">WORKBENCH <span id="editor-dna" style="font-size:0.7em; color:#fff; margin-left:10px;">DNA: 0</span></div>
                    <button id="close-editor-btn" class="btn-neon wb-btn" style="border-color:#f0f; color:#f0f; font-size:0.8rem; padding:5px 15px;">PLAY</button>
                </div>
                <div id="workbench-controls" style="display:flex; gap:10px;">
                    <button id="add-vertebra-btn" class="btn-neon wb-btn">+ BONE (10)</button>
                    <button id="toggle-mirror-btn" class="btn-neon wb-btn">SYM: OFF</button>
                    <button id="toggle-sell-btn" class="btn-neon wb-btn" style="border-color:#fa0; color:#fa0;">SELL: OFF</button>
                </div>
            </div>

            <!-- Zoom Controls -->
            <div style="position:absolute; top:130px; right:20px; display:flex; flex-direction:column; gap:10px; pointer-events:auto;">
            ...
            <div id="editor-instruction" style="position:absolute; top:110px; width:100%; text-align:center; color:rgba(255,255,255,0.5); font-size:0.8rem; pointer-events:none;">
                DRAG PARTS TO BODY • TAP BONE TO RESIZE
            </div>
        `;
        document.body.appendChild(this.overlay);

        // UI LAYOUT FIX (CSS Injection)
        const controls = document.getElementById('workbench-controls');
        if(controls) {
    ...
    findClosestBodyNode(mx, my) {
        // Find closest node based on screen position
        const cam = this.game.camera;
        let closest = null;
        let minDist = Infinity;

        if (!this.clone || !this.clone.points) return null;
        ...
        // Threshold: 300px radius for interaction
    ...
    endDrag(e) {
        ...
        const closest = this.findClosestBodyNode(this.dragX, this.dragY);

        if (closest) {
            // STRICT MATH LOGIC (Snapping)
            const cam = this.game.camera;
            const mouseWorld = cam.screenToWorld(this.dragX, this.dragY);
            const node = closest.point;
            ...
            // Add Part
            this.addPart(this.selectedPart, closest.index, angle);

            // Symmetry Logic: Mirror Angle across Y Axis (PI - angle)
            if (this.symmetry) {
                let mirrorAngle = Math.PI - angle;
                // Normalize Angle
    ...
    render(ctx) {
        ...
        if (this.isDragging && this.selectedPart) {
            ...
            if (closest) {
                ...
                // Position forced to radius: x = node.x + cos(angle) * radius
                const ghostWorldX = node.x + Math.cos(angle) * node.radius;
                const ghostWorldY = node.y + Math.sin(angle) * node.radius;

                // Rotation forced: angle + PI/2
                const ghostRotation = angle + Math.PI/2;

                const ghostScreen = cam.worldToScreen(ghostWorldX, ghostWorldY);
                ...
                // Draw simple shape for ghost
                ctx.fillStyle = 'rgba(0, 255, 255, 0.5)';
                ctx.beginPath();
                ctx.moveTo(0, -10); ctx.lineTo(10, 10); ctx.lineTo(-10, 10);
                ctx.fill();

                ctx.restore();

                // Draw Snap Circle on Node
                ctx.save();
                ctx.strokeStyle = '#0ff';
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.arc(closest.sp.x, closest.sp.y, node.radius * cam.zoom * 1.2, 0, Math.PI*2);
                ctx.stroke();
                ctx.restore();
            }
            ...
            ctx.fillText(this.selectedPart, this.dragX + 15, this.dragY);
            ctx.restore();
        }

        if (this.selectedBone && !this.isDragging) {
            const cam = this.game.camera;
    ...
    exitPetriMode() {
        ...
        if (this.game.mate) this.game.mate = null;
    ...
    onCanvasClick(e) {
        ...
        const pt = this.getEventPos(e);
        const closest = this.findClosestBodyNode(pt.x, pt.y);

        if (closest) {
            const parts = this.clone.parts || [];
            const boneParts = parts.filter(p => p.boneIndex === closest.index);
            ...
            if (this.sellMode) {
                 ...
                 this.selectedBone = closest;
                 this.showResizeSlider(closest);
            }
        } else {
    ...
    showResizeSlider(selection) {
        ...
        if (!slider) {
            ...
            document.getElementById('bone-scale').addEventListener('input', (e) => {
                if (this.selectedBone) {
                    ...
                    if (!this.selectedBone.point.initialBaseRadius) this.selectedBone.point.initialBaseRadius = this.selectedBone.point.baseRadius;
                    const newRadius = this.selectedBone.point.initialBaseRadius * scale;
                    this.selectedBone.point.baseRadius = newRadius;
                    this.selectedBone.point.radius = newRadius;
                    this.selectedBone.point.scaleFactor = scale;
                }
            });
        }
        ...
        if (selection.point.scaleFactor === undefined) {
             const base = selection.point.initialBaseRadius || 20;
             selection.point.scaleFactor = selection.point.baseRadius / base;
        }
        const currentScale = selection.point.scaleFactor || 1.0;
